...
echo ""
echo "Deploy with:"
echo "sam deploy --template-file deploy/template.yaml --stack-name invoice-tax-lambda-server --capabilities CAPABILITY_IAM CAPABILITY_NAMED_IAM --resolve-image-repos --parameter-overrides ImageUri=${IMAGE_URI} ApiKey=juno_labs_ai"